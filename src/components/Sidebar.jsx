...
export default function Sidebar({ activeTab, onTabChange, userName, currentStreak, onLogout }) {
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 fixed left-0 top-0 bottom-0 z-40 bg-dark-950/90 border-r border-white/5 backdrop-blur-xl p-4 justify-between">
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {isActive && (
                  <div className="absolute left-0 top-2 bottom-2 w-1 bg-accent-primary rounded-r-full shadow-glow" />
                )}
                ...
                {item.badge && (
                  <span className="text-[9px] font-black px-1.5 py-0.5 rounded bg-accent-primary text-dark-900 shadow-glow">
                    {item.badge}
                  </span>
                )}
              </button>
      ...
      <div className="pt-4 border-t border-white/5 space-y-3">
        <div className="flex items-center justify-between px-2">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-accent-primary font-bold text-xs">
