...
export default function TopHeaderBar({ userName, selectedDate, onSelectDate, currentStreak }) {
  ...
  return (
    <header className="w-full bg-dark-900/90 border-b border-white/10 backdrop-blur-xl py-3 px-4 sm:px-6 mb-6 rounded-2xl flex items-center justify-between shadow-lg">
      
      {/* Left: Brand & Date Navigator */}
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-accent-primary to-cyan-400 flex items-center justify-center text-dark-900 font-black text-sm shadow-glow">
            ⚡
          </div>
          <div>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/5 text-xs">
          <div className="w-6 h-6 rounded-full bg-accent-primary/20 text-accent-primary font-bold flex items-center justify-center text-[11px]">
          ...
          <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${isGuest ? 'bg-amber-500/20 text-amber-300' : 'bg-emerald-500/20 text-emerald-300'}`}>
        ...
        <button
          ...
          className={`px-4 py-2 rounded-xl text-xs font-extrabold flex items-center gap-1.5 transition-all shadow-glow ${
            isGuest 
              ? 'bg-accent-primary text-dark-900 hover:brightness-110' 
