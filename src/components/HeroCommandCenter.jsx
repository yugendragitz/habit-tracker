...
export default function HeroCommandCenter({
  ...
  return (
    <div className="card p-6 sm:p-8 bg-gradient-to-br from-dark-800/90 via-dark-800/80 to-dark-900 border-white/10 shadow-2xl relative overflow-hidden space-y-6">
      
      {/* Background Subtle Ambient Spotlight */}
      <div className="absolute -top-24 -right-24 w-72 h-72 bg-accent-primary/10 rounded-full blur-3xl pointer-events-none" />

      {/* Top Greeting & Date Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div>
          <span className="text-[10px] font-bold text-accent-primary uppercase tracking-widest block mb-1">PERSONAL COMMAND CENTER</span>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="flex items-baseline justify-end gap-1">
              <span className="text-3xl font-black text-white">{transformationScore}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
        ...
        <div className="md:col-span-2 grid grid-cols-2 gap-3 text-xs">
          ...
          <div className="p-4 rounded-2xl bg-dark-900/80 border border-white/5 space-y-1">
            <span className="text-[10px] text-white/40 font-bold uppercase tracking-wider block">Habit Routine</span>
            <div className="flex items-baseline justify-between pt-1">
              <strong className="text-white text-base font-extrabold">{completedHabitsCount} / {totalCount}</strong>
              <span className="text-xs text-accent-primary font-bold">{Math.round((completedHabitsCount / (totalCount || 1)) * 100)}%</span>
            </div>
          ...
          <div 
            ...
            className="p-4 rounded-2xl bg-dark-900/80 border border-white/5 space-y-1 cursor-pointer hover:border-accent-primary/40 transition-all group"
          >
            <span className="text-[10px] text-white/40 font-bold uppercase tracking-wider block">Gym / Fitness</span>
            <div className="flex items-baseline justify-between pt-1">
              <strong className="text-white text-base font-extrabold">
                {currentWorkouts.length > 0 ? `✓ ${currentWorkouts.length} Logged` : '○ Not Logged'}
              </strong>
              <span className="text-xs text-accent-primary font-bold group-hover:underline">+ Log</span>
            </div>
          ...
          <div 
            ...
            className="p-4 rounded-2xl bg-dark-900/80 border border-white/5 space-y-1 cursor-pointer hover:border-accent-primary/40 transition-all"
          >
            <span className="text-[10px] text-white/40 font-bold uppercase tracking-wider block">Daily Calories</span>
            <div className="flex items-baseline justify-between pt-1">
            ...
            <p className="text-[11px] text-emerald-400 font-bold pt-1">{currentDailyNutrition.totalProtein}g protein logged</p>
          </div>
          ...
          <div 
            ...
            className="p-4 rounded-2xl bg-dark-900/80 border border-white/5 space-y-1 cursor-pointer hover:border-accent-primary/40 transition-all"
          >
            <span className="text-[10px] text-white/40 font-bold uppercase tracking-wider block">Hydration Goal</span>
            <div className="flex items-baseline justify-between pt-1">
